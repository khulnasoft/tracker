...
func TestSetupTrackerGobInputSource(t *testing.T) {
	testCases := []struct {
		testName      string
		events        []trace.Event
		expectedError error
	...
	for _, testCase := range testCases {
		t.Run(testCase.testName, func(t *testing.T) {
			// Setup temp file that tracker-rules reads from
			f, err := os.CreateTemp("", "TestSetupTrackerGobInputSource-")
			if err != nil {
				t.Error(err)
			}
			defer func() {
			...
			// Set up reading from the file
			opts := &trackerInputOptions{inputFile: f, inputFormat: gobInputFormat}
			eventsChan, err := setupTrackerGobInputSource(opts)
			assert.Equal(t, testCase.expectedError, err)
