...
import (
	...
	"github.com/khulnasoft/tracker/pkg/events/parsers" // Add this import
...
func (t *Tracker) Init(ctx gocontext.Context) error {
	...
	err = trackertime.Init(int32(usedClockID))
	if err != nil {
		return errfmt.WrapError(err)
	}

	// elapsed time in nanoseconds since system start
	t.startTime = uint64(trackertime.GetStartTimeNS())
	// time in nanoseconds when the system was booted
	t.bootTime = uint64(trackertime.GetBootTimeNS())
...
func (t *Tracker) invokeInitEvents(out chan *trace.Event) {
	...
	matchedPolicies = policiesMatch(events.TrackerInfo)
	if matchedPolicies > 0 {
		trackerDataEvent := events.TrackerInfoEvent(t.bootTime, t.startTime)
		setMatchedPolicies(&trackerDataEvent, matchedPolicies)
