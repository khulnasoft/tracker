...
func (ti *TaskInfo) SetExitTime(exitTime uint64) {
	...
	exitTimestamp := trackertime.NsSinceEpochToTime(exitTime)
...
func (ti *TaskInfo) GetStartTime() time.Time {
	...
	return trackertime.NsSinceEpochToTime(ti.getFeed().StartTimeNS)
}
...
func (ti *TaskInfo) GetExitTime() time.Time {
	...
	return trackertime.NsSinceEpochToTime(ti.getFeed().ExitTimeNS)
}
...
func (ti *TaskInfo) IsAliveAt(targetTime time.Time) bool {
	...
	if exitTimeNS != 0 {
		if targetTime.After(trackertime.NsSinceEpochToTime(exitTimeNS)) {
			return false
		}
	}
	...
	// query time given.
	return !targetTime.Before(trackertime.NsSinceEpochToTime(feed.StartTimeNS))
}
